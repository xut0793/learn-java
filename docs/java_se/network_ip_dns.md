...
Java 的 `java.net` 包中提供了 `InetAddress` 类，用于处理 IP 地址和 DNS 解析。

`InetAddress` 类封装了 IP 地址和对应的主机名，它没有公共构造函数，必须通过静态方法获取实例。

- 域名解析为 IP（正向解析）：`InetAddress.getByName("www.example.com")` 获取指定主机名对应的一个 IP 地址。
- IP 解析为主机名（反向解析）：`InetAddress.getHostName()` 方法可以根据 IP 地址查找对应的主机名。但需要注意，这依赖于 DNS 的 PTR 记录，如果未配置，可能直接返回 IP 字符串。
- 获取本机 IP：`InetAddress.getLocalHost()`：这是最简单的方式，但在多网卡或 hosts 文件配置不当的环境下，极易返回 `127.0.0.1` 或 `localhost`，而不是真实的对外网卡 IP。
...
public class InetAddressDemo {
    public static void main(String[] args) {
        // DNS 解析涉及网络 I/O，务必使用 try-catch 捕获 UnknownHostException，防止程序因域名失效而崩溃。
       try {
